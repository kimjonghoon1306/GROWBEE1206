...
(function () {
  ...
  function adminCommandCenter() {
    ...
    bar.innerHTML = '<strong>🎯 운영 작업함</strong><span>마감·승인·검수·정산을 한 흐름으로 관리하세요.</span><a href="admin-review.html">검수 대기</a><a href="admin-advertisers.html">광고주 승인</a><a href="admin-settlements.html">정산 대기</a><a href="admin-inquiries.html">문의 처리</a><a href="admin-content.html">CMS·신고</a>';
  ...
  async function missionPanel(){if(!/reward\.html$/.test(location.pathname)||!window.OnAuth)return;var main=document.querySelector('main,.reward-wrap,.container');if(!main)return;try{if(!await OnAuth.getUser())return;var rows=await OnAuth.myMissions(),s=document.createElement('section');s.className='oc-missions';s.innerHTML='<h2>🎯 나의 미션</h2><div>'+rows.map(function(m){var pct=Math.min(100,Math.round((m.progress||0)/Math.max(1,m.target)*100));return '<article><b>'+esc(m.title)+'</b><span>'+esc(m.description)+' · +'+Number(m.reward_points||0).toLocaleString()+'P</span><div><i style="width:'+pct+'%"></i></div><small>'+m.progress+'/'+m.target+(m.completed?' · 완료':'')+'</small>'+(m.completed&&!m.claimed?'<button data-mission="'+m.id+'">보상 수령</button>':(m.claimed?'<em>수령 완료</em>':''))+'</article>';}).join('')+'</div>';main.insertBefore(s,main.firstChild);s.querySelectorAll('button[data-mission]').forEach(function(b){b.onclick=async function(){this.disabled=true;var r=await OnAuth.claimMissionReward(this.dataset.mission);if(r&&r.ok){toast(Number(r.points||0).toLocaleString()+'P를 적립했어요.');this.outerHTML='<em>수령 완료</em>';}else{toast((r&&r.msg)||'보상 수령 실패');this.disabled=false;}};});}catch(_){}}
  async function notificationSettings(){if(!/messages\.html$/.test(location.pathname)||!window.OnAuth)return;var main=document.querySelector('.mp-main,main');if(!main)return;var p=await OnAuth.getNotificationPreferences()||{in_app:true,email:true,web_push:false,marketing:false},s=document.createElement('section');s.className='mp-card oc-notify-settings';s.innerHTML='<h2>알림 수신 설정</h2><label><input type="checkbox" data-k="in_app" '+(p.in_app?'checked':'')+'> 앱 알림</label><label><input type="checkbox" data-k="email" '+(p.email?'checked':'')+'> 이메일</label><label><input type="checkbox" data-k="web_push" '+(p.web_push?'checked':'')+'> 브라우저 푸시</label><label><input type="checkbox" data-k="marketing" '+(p.marketing?'checked':'')+'> 맞춤 캠페인·이벤트</label>';main.insertBefore(s,main.firstChild);s.onchange=async function(){var f={};s.querySelectorAll('input').forEach(function(x){f[x.dataset.k]=x.checked;});if(f.web_push&&'Notification'in window&&Notification.permission==='default')f.web_push=(await Notification.requestPermission())==='granted';await OnAuth.saveNotificationPreferences(f);toast('알림 설정을 저장했어요.');};}
  async function contentLibrary(){if(!/advertiser-dashboard\.html$/.test(location.pathname)||!window.OnAuth)return;var app=document.getElementById('adv-app');if(!app)return;try{var rows=await OnAuth.myContentLicenses(),s=document.createElement('section');s.className='oc-library';s.innerHTML='<h2>🎨 콘텐츠 사용권 라이브러리</h2><p>광고 소재로 활용 동의된 완료 콘텐츠와 사용기간을 관리합니다.</p><div>'+(rows.length?rows.map(function(x){return '<a href="'+esc(x.review_url)+'" target="_blank" rel="noopener"><b>'+esc(x.title)+'</b><span>'+(x.ends_at?new Date(x.ends_at).toLocaleDateString('ko-KR')+'까지':'기간 제한 없음')+' · '+esc((x.usage_scope||[]).join(', ')||'활용범위 확인')+'</span></a>';}).join(''):'<div class="oc-task-empty">사용권이 확정된 콘텐츠가 없어요.</div>')+'</div>';app.appendChild(s);}catch(_){}}
  function addCommunityNav(){if(location.pathname.indexOf('/pages/user/')<0)return;document.querySelectorAll('.nav-links').forEach(function(nav){if(nav.querySelector('a[href="community.html"]'))return;var a=document.createElement('a');a.href='community.html';a.className='nav-link';a.textContent='커뮤니티';nav.appendChild(a);});}
  ...
  function boot() { observeCards(); detailTools(); addGlobalSearch(); installErrorBoundary(); adminCommandCenter(); reviewerTaskBoard(); advertiserReport(); snsChannelManager(); syncCloudState(); missionPanel(); notificationSettings(); contentLibrary(); addCommunityNav(); }
